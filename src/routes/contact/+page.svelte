...
<section class="max-w-2xl mx-auto py-16 px-6 min-h-screen bg-[#0d1117] text-slate-200 border border-[#202428]">
  ...
  <form on:submit={handleSubmit} class="space-y-4">
    <div>
      ...
      <input bind:value={name} class="w-full px-4 py-3 rounded-lg bg-[#071019] border border-[#1f2a2f] text-slate-200" placeholder="Your name" />
    </div>

    <div>
      <label class="block text-sm font-medium text-slate-300 mb-1">Email</label>
      <input type="email" bind:value={email} class="w-full px-4 py-3 rounded-lg bg-[#071019] border border-[#1f2a2f] text-slate-200" placeholder="you@example.com" />
    </div>

    <div>
      <label class="block text-sm font-medium text-slate-300 mb-1">Message</label>
      <textarea bind:value={message} rows="6" class="w-full px-4 py-3 rounded-lg bg-[#071019] border border-[#1f2a2f] text-slate-200" placeholder="How can we help?"></textarea>
    </div>

    <div class="flex items-center gap-4">
      <button type="submit" class="px-5 py-2 bg-[#2563EB] text-white rounded-lg" disabled={loading}>
